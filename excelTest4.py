...
# 엑셀파일 열기
wb1 = openpyxl.load_workbook('합계3.xlsx')
wb2 = openpyxl.load_workbook('20200828_정답회신리스트.xlsx')
wb3 = Workbook()
file_name = '합계4.xlsx'
ws3 = wb3.active
ws3.title = '합계4'
...
for item in new_list:
    ...
    for r2 in ws2.rows:
        if r2[0].row > 1:
            ...
            w2score = 0
            if 'int' == type(r2[7].value):
                w2score = r2[7].value
            elif r2[7].value is not None:
                w2score = int(r2[7].value)
            if item == w2personName:
                # print(total)
                # print(type(r2[8].value))
                # print(w2score)
                departmentName = r2[4].value
                total = total + w2score
                attend = attend + 1

    row_index = new_list.index(item) + 2
    ws3.cell(row=row_index, column=4).value = departmentName
    ws3.cell(row=row_index, column=5).value = item
    ws3.cell(row=row_index, column=6).value = total
    ws3.cell(row=row_index, column=7).value = attend
